    ...
    public class BlowfishAlgorithm : SymmetricAlgorithm, ICryptoTransform
    {
        ...
        void CopyPadding(BlowfishAlgorithm ba)
        {
            switch (Padding)
            {
                ...
                default:
                {
                    throw new CryptographicException(String.Format("Unsupported padding mode {0}.", Padding));
                }
            }
        }
        ...
        public byte[] TransformFinalBlock(byte[] buf, int ofs, int count)
        {
            ...
            {
                throw new CryptographicException(String.Format("Unexpected last block count {0}.", count));
            }
            ...
            if (this.isEncryptor)
            {
                ...
                {
                    throw new CryptographicException(String.Format("Unknown padding {0} for final encryption.", PaddingValue));
                }
            ...
            else
            {
                ...
                if (PaddingMode.PKCS7 == PaddingValue)
                {
                    ...
                    {
                        throw new CryptographicException(String.Format("Invalid PKCS7 padding value {0}.", pval));
                    }
                    ...
                    for (int i = tmp.Length - 2; i >= rest; i--)
                    {
                        if (tmp[i] != pval)
                        {
                            throw new CryptographicException(String.Format("Invalid PKCS7 padding data {0} at {1}.", pval, i));
                        }
                    }
                }
                else if (PaddingMode.Zeros == PaddingValue ||
                ...
                else if (PaddingMode.ANSIX923 == PaddingValue)
                {
                    ...
                    {
                        throw new CryptographicException(String.Format("Invalid ANSIX923 padding value {0}.", pval));
                    }
                    rest = tmp.Length - pval;
                    for (int i = tmp.Length - 2; i >= rest; i--)
                    {
                        if (tmp[i] != 0)
                        {
                            throw new CryptographicException(String.Format("Nonzero ANSIX923 padding data {0} at {1}.", pval, i));
                        }
                    }
                }
                else
                {
                    throw new CryptographicException(String.Format("Unknown padding {0} for final decryption.", PaddingValue));
                }
